...
% code to display the spectrogram of a discrete-time signal:
...
for i=0:num_seg-1
    ...
    in_sig_wind = x((1+i*shift) : (win_len+i*shift)).*win_sig;
...
time = ((win_len/2) : shift : (win_len/2 + (num_seg-1)*shift)) / freq;
freqs = (0 : num_uniq_fft-1) * (freq/num_fft);
